...
// Esto debe moverse a un objeto de configuración
			...
			var puzInit = {
				...
				init: function() {
					currOri = 'across'; // la orientación inicial de la aplicación podría moverse al objeto de configuración
				...
				calcCoords: function() {
					...
					*/
					for (var i = 0, p = entryCount; i < p; ++i) {		
						...
						if (puzz.data[i].hasOwnProperty('imagen')) {
							let reino = puzz.data[i].answer;
							let img = Math.floor((Math.random() * (15 - 1 + 1)) + 1);
							$('#' + puzz.data[i].orientation).append('<li style="cursor:pointer" tabindex="1" data-position="' + i + '">(' + puzz.data[i].position + ') ' + puzz.data[i].clue + '<img width="60" class="nombreCat1" style="margin-left:10px;"  src="img/reinos/'+reino+'/'+img+'.png" /></li>'); 
				...
				checkAnswer: function(e) {
					...
					if(valToCheck === currVal){	
						...
						if(encontradas == entryData.length){
							...
							document.getElementById("texto_final").innerText = "Felicitaciones, has contestado correctamente"
