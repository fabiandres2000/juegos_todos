...
function obtenerIndiceDivMost() {
    let indice = Math.floor(Math.random() * (19 - 1 + 1) + 1);
    while (DivMostrado.includes(indice)) { 
        indice = Math.floor(Math.random() * (19 - 1 + 1) + 1);
    }
...
next_btn.onclick = ()=>{
    if(nPreg <= 20){ //if question count is less than total question length
        que_count = obtenerIndiceAleatorio();
...
function optionSelected(answer){
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        ...
        mostrarPartImg(); //llama funcion para ir mostrando una parte de la imagen
        if(userScore>4){
            document.getElementById("opcHominido").style.opacity = 1;
        }
    }else{
...
function startTimerLine(time){
    ...
    function timer(){
        ...
        if(time > 699){ //if time value is greater than 549
            clearInterval(counterLine); //clear counterLine
        }
    }
}
...
function queCounter(index){
    ...
    let totalQueCounTag = '<span><p>'+ index +'</p> de <p>20</p> Preguntas</span>';
    bottom_ques_counter.innerHTML = totalQueCounTag;  //adding new span tag inside bottom_ques_counter
}



function mostrarPartImg(){
    let index = obtenerIndiceDivMost();
